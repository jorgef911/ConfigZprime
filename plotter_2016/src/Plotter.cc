...
void Plotter::CreateStack( TDirectory *target, Logfile& logfile) {
  ...
  Int_t cVV = 46; // color index                                                                                                          
  TColor *colorVV = new TColor(cVV, 1,0,0 );

  Int_t cWJet = 62; // color index                                                                                                     
  TColor *colorWJet = new TColor(cWJet, 1, 0, 0 );

  Int_t cTT = 38; // color index                                                                                                        
  TColor *colorTT = new TColor(cTT, 0, 0, 1 );

  Int_t cQCD = 6; // color index                                                                                                        
  TColor *colorQCD = new TColor(cQCD, 250.0/255.0, 202.0/255.0, 255.0/255.0);

  Int_t cDY = 42; // color index                                                                                                          
  ...
  if(events) {
	...
	/*
	gDirectory->GetObject("NVertices", events);
        double err;
        double nevents = events->IntegralAndError(-1,-1,err);
        totalval = to_string_with_precision(nevents, 1);
	*/
  ...
  while ( (key = (TKey*)nextkey())) {
	...
	while ( nextfile ) {
     ...
     /*------------background--------------*/
		...
		h2->SetBinError(j, 0);
	      ...
	      //cout << "BKG " << nextfile->GetTitle() << endl;
	      if(strcmp(nextfile->GetTitle(),"QCD.root")==0){
		h2->SetFillColor(kPink+1);
	      }else if(strcmp(nextfile->GetTitle(),"DY+Jets.root")==0){
		h2->SetFillColor(kOrange-2);
	      }else if(strcmp(nextfile->GetTitle(),"VV.root")==0){
		h2->SetFillColor(kRed-3);
	      }else if(strcmp(nextfile->GetTitle(),"W+Jets.root")==0){
		h2->SetFillColor(kAzure+6);
	      }else if(strcmp(nextfile->GetTitle(),"tt.root")==0){
		h2->SetFillColor(kBlue-8);
	      }else if(strcmp(nextfile->GetTitle(),"SingleTop.root")==0){
     ...
     /*------------Signal--------------*/
      ...
      cout << hs->GetTitle() << endl;
      cout << "**********************************" << hs->GetNhists() << endl;
      /// sort based on integral.  Change this function is want other order
      if(strcmp(hs->GetTitle(),"DiTauReconstructableMass")==0){
	cout << "ESTE ES " << "======================" << hs->GetNhists() << endl;
      }
      hs = sortStack(hs);
      cout << "**********************************" << hs->GetNhists() << endl;
      ///rebin
	...
	tmpsig = (TH1D*)sigHists->After(tmpsig);
      ...
      if(explicitBins.find(readObj->GetTitle()) == explicitBins.end()){
	...
	//	cout << " Histo " << readObj->GetTitle() << endl;
	...
	continue;
      ...
      /*
      if(strcmp(readObj->GetTitle(),"DiTauReconstructableMass")==0){
	cout << " NO REBINNED Histo " << readObj->GetTitle() << endl;
	...
	/*
	if(strcmp(readObj->GetTitle(),"DiTauReconstructableMass")==0){
	  cout << " NO DIVIDED Histo " << readObj->GetTitle() << endl;
	  for(int b=1; b<error->GetXaxis()->GetNbins();b++){
	    cout << " b " << b << "  " << error->GetXaxis()->GetBinLowEdge(b) << "-" << error->GetXaxis()->GetBinLowEdge(b) + error->GetXaxis()->GetBinWidth(b) << " BinContent " << error->GetBinContent(b) << " Error " << error->GetBinError(b) << endl;
	  }
	}
...
TLegend* Plotter::createLeg(const TH1* data, const TList* bgl, const TList* sigl) {
  ...
  while(tmp) {
    if(strcmp(tmp->GetTitle(),"tt")==0){
      leg->AddEntry(tmp, "t#bar{t}", "f");
    }else{
  ...
  tmp = (TH1*)sigl->First();
  while(tmp) {
    if(strcmp(tmp->GetTitle(),"LSP_150")==0){
     leg->AddEntry(tmp, "m(#tilde{#chi_{0}^{1}})=150 GeV, m(#tilde{#chi_{1}^{#pm}})=200 GeV, m(#tilde{#tau})=175 GeV", "lep");
	      }

    else  leg->AddEntry(tmp, tmp->GetTitle(), "lep");
